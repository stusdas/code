...
def main():
    ...
    try:
        ...
        st.sidebar.warning(f"无法从 config.py 加载配置: {e}")
        default_config = Config()

    with st.sidebar:
        st.header("配置")
        deepseek_key = st.text_input("DeepSeek API Key", value=default_config.deepseek_api_key or "", type="password")
        tavily_key = st.text_input("Tavily API Key", value=default_config.tavily_api_key or "", type="password")
        llm_provider = st.selectbox("LLM提供商", ["deepseek", "openai"], index=0 if default_config.default_llm_provider == "deepseek" else 1)
        
        openai_key = ""
        if llm_provider == "openai":
            openai_key = st.text_input("OpenAI API Key", value=default_config.openai_api_key or "", type="password")
            model_name = st.selectbox("OpenAI模型", ["gpt-4o", "gpt-4o-mini"], index=0 if default_config.openai_model == "gpt-4o" else 1)
        else:
            model_name = st.selectbox("DeepSeek模型", ["deepseek-chat", "deepseek-reasoner"], index=0 if default_config.deepseek_model == "deepseek-chat" else 1)
            
        max_reflections = st.slider("反思次数", 1, 5, default_config.max_reflections)
        max_search_results = st.slider("搜索结果数", 1, 10, default_config.max_search_results)
