...
并支持 progress_callback，供 Streamlit 等 UI 实时展示执行过程
"""
...
class DeepSearchAgent:
    ...
    def __init__(
        ...
        Args:
            config: 配置对象，如果不提供则自动加载
            progress_callback: UI层回调，用于实时展示执行过程
        """
        self.config = config or load_config()
        self.llm_client = self._initialize_llm()
        self._initialize_nodes()
    ...
    def _emit_progress(
        ...
        """
        向UI发送实时进度事件
        stage: plan / structure / search / reflection / report / review / save
        message: 用户可见的简短描述
        data: 结构化细节
        """
        payload = {
        ...
        if self.progress_callback:
            try:
                ...
                print(f"[警告] progress_callback 调用失败: {e}")
    ...
    def research(self, query: str, save_report: bool = True) -> str:
        ...
        try:
            ...
            # Step 0: plan
            self._emit_progress("plan", "正在生成研究计划...")
            with self.tracer.span("step0.make_research_plan"):
                memory_record = self.memory.find_similar_query(query, threshold=0.85)
                if memory_record and memory_record.get("plan"):
                    plan = memory_record["plan"]
            ...
            # Step 1: structure
            self._emit_progress("structure", "正在生成报告结构...")
            with self.tracer.span("step1.generate_report_structure"):
                self._generate_report_structure(query, plan)
                self.tracer.log_event(
                    "info",
            ...
            # Step 2: search + reflection
            self._emit_progress("search", "正在进入段落研究流程...")
            with self.tracer.span("step2.process_paragraphs"):
                self._process_paragraphs(plan)

            # Step 3: report
            self._emit_progress("report", "正在生成初版报告...")
            with self.tracer.span("step3.generate_draft_report"):
                draft_report = self._generate_final_report()

            # Step 4: review
            self._emit_progress("review", "正在进行全文总审...")
            with self.tracer.span("step4.global_review"):
                final_report = self._global_review(query, draft_report, plan)

            # Step 5: save
            if save_report:
                self._emit_progress("save", "正在保存最终报告...")
                with self.tracer.span("step5.save_report"):
                    self._save_report(final_report)
            ...
            self._emit_progress(
                ...
                "研究完成",
                {
                    "final_report_preview": final_report[:1500]
                }
            )
    ...
    def _generate_report_structure(self, query: str, plan: Dict[str, Any]):
        ...
        self._emit_progress(
            "structure",
            f"报告结构生成完成，共 {len(self.state.paragraphs)} 个段落",
            {
                ...
                "paragraphs": [
                    ...
                    for i, p in enumerate(self.state.paragraphs)
                ]
            }
        )
    ...
    def _process_paragraphs(self, plan: Dict[str, Any]):
        ...
        for i in range(total_paragraphs):
            ...
            self._emit_progress(
                "search",
                f"开始处理段落 {i + 1}/{total_paragraphs}: {self.state.paragraphs[i].title}",
                {
                    "paragraph_index": i,
                    "paragraph_title": self.state.paragraphs[i].title,
                    "paragraph_content": self.state.paragraphs[i].content
                }
            )
            ...
            self._emit_progress(
                "search",
                f"段落处理完成: {self.state.paragraphs[i].title}",
                {
                    "paragraph_index": i,
    ...
    def _initial_search_and_summary(self, paragraph_index: int, plan: Dict[str, Any]):
        ...
        self._emit_progress(
            "search",
            f"搜索完成，找到 {len(search_results or [])} 个结果",
            {
                "section_title": section_title,
                "search_query": search_query,
                "results": [
        ...
        self._emit_progress(
            ...
            {
                "section_title": section_title,
                "latest_summary_preview": self.state.paragraphs[paragraph_index].research.latest_summary[:500]
            }
        )
    ...
    def _reflection_loop(self, paragraph_index: int, plan: Dict[str, Any]):
        ...
        for reflection_i in range(self.config.max_reflections):
            ...
            self._emit_progress(
                "reflection",
                f"开始第 {reflection_i + 1}/{self.config.max_reflections} 轮反思",
                {
                    "section_title": section_title,
                    "iteration": reflection_i + 1
                }
            ...
            self._emit_progress(
                "reflection",
                f"反思搜索完成，找到 {len(search_results or [])} 个结果",
                {
                    "section_title": section_title,
                    "iteration": reflection_i + 1,
                    "results": [
    ...
    def _global_review(self, query: str, draft: str, plan: Dict[str, Any]) -> str:
        ...
        self._emit_progress(
            ...
            "全文总审完成",
            {
                "final_report_preview": final[:1000]
            }
        )
